        ...
        >
            ...
            {({ geographies }: { geographies: { rsmKey: string; id: string }[] }) =>
              geographies.map((geo) => (
                <Geography
                  ...
                  geography={geo}
                  style={{
                    default: { fill: '#e5e7eb', stroke: '#fff', strokeWidth: 0.5, outline: 'none' },
          ...
          {peaks.map((peak) => (
            <Marker
              ...
              onMouseEnter={() => setTooltip(`${peak.name} · ${peak.elevation} м`)}
              onMouseLeave={() => setTooltip(null)}
            >
              {/* Треугольник-горка */}
              <polygon
                points={`0,${-10 / zoom} ${-6 / zoom},${2 / zoom} ${6 / zoom},${2 / zoom}`}
                fill="#111"
